...
# Window 1: main - shell on the left, zpool iostat on the right
main_pane=$(tmux display-message -p -t "$SESSION_NAME:main" "#{pane_id}")
zpool_pane=$(tmux split-window -h -p 34 -c "$HOME" -t "$main_pane" -P -F "#{pane_id}")

# Pane 2: zpool iostat watch command
...
# Select the first window and zpool pane, matching the current h12-truenas session
tmux select-window -t "$SESSION_NAME:main"
tmux select-pane -t "$zpool_pane"
