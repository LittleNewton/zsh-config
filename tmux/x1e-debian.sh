...
# Create new session with first window named 'zsh'
tmux new-session -d -s "$SESSION_NAME" -n zsh -c "$HOME" -x "$(tput cols)" -y "$(tput lines)"

# Window 1: zsh - shell/iotop on the left, zpool/iotop on the right
shell_pane=$(tmux display-message -p -t "$SESSION_NAME:zsh" "#{pane_id}")
zpool_pane=$(tmux split-window -h -p 28 -c "$HOME" -t "$shell_pane" -P -F "#{pane_id}")
tmux resize-pane -t "$shell_pane" -x 137
...
tmux resize-pane -t "$zpool_pane" -y 5
...
# Select the first window and shell pane, matching the current x1e-debian session
