...
right_iotop_pane=$(tmux split-window -v -p 82 -c "$HOME" -t "$zpool_pane" -P -F "#{pane_id}")
...
pools="zroot Samsung_970EvoPlus_Stripe"
...
# Select the first window and shell pane, matching the current x299-debian session
